...
async function setSecondDNS() {
    ...
    const secondDnsData = await tools.proxyRequest(
        url,
        'get',
        null,
        null,
        'http://127.0.0.1:' + port,
        __dirname + '/../' + config.cert_file,
    );
            ...
            if (req.url.indexOf(config.dns_server) >= 0 || req.url.indexOf(config.alias_server) >= 0) {
                serverSocket.on('data', function (data) {
                    if (data.toString() == 'OK') {
                        ...
                        logger.debug(
                            `v0 secret key string: ${Buffer.from(secHash, 'hex').subarray(0, 32).toString('hex')}`,
                        );
                ...
                serverSocket.on('connect', () => {
                    clientSocket.write(
                        ['HTTP/1.1 200 Connection Established', 'Proxy-agent: Node-heliumos-proxy'].join('\r\n'),
                    );
                    clientSocket.write('\r\n\r\n');
                ...
                serverSocket.on('data', function (data) {
                    if (data.toString() == 'OK') {
                        serverSocket.write(JSON.stringify(handshakeData));
                    } else if (!publicKeyJson) {
                        // 可能会出现公钥data被截断的情况（导致JSON.parse报错），需要将两次data拼接到一起，生成公钥
                        try {
                            keyData = Buffer.concat([keyData, data]);
                            publicKeyJson = JSON.parse(keyData.toString());
                        } catch (error) {
                            publicKeyJson = null;
                            return;
                        }

                        // publicKey接收成功后，再传输数据
                        let sec = ecdh.computeSecret(publicKeyJson.data, 'hex');
                        const hash = crypto.createHash('sha256');
                        hash.update(sec);
                        const secHash = hash.digest('hex');
                        secretKey = Buffer.from(secHash, 'hex').subarray(0, 32);
                        // logger.debug(
                        //     `v1 secret key string: ${Buffer.from(secHash, 'hex').subarray(0, 32).toString('hex')}`,
                        // );
                        serverSocket.write('OK', 'utf8');
...
async function updateAliasDb() {
    ...
    if (env.indexOf('custom') === 0) {
        ...
        const aliasData = await tools.proxyRequest(
            url,
            'get',
            null,
            null,
...
async function fetch(reqUrl, method, headers, body) {
    ...
    if (dnsMap.has(org)) {
        const data = await tools.proxyRequest(
            reqUrl,
            method,
            headers,
            body,
